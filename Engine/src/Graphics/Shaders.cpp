...
bool VertexShader::Init(std::wstring aFilePath, D3D11_INPUT_ELEMENT_DESC* aDesc, UINT aDescSize)
{
	...
	hr = DX11::myDevice->CreateVertexShader(myShaderBuffer->GetBufferPointer(), myShaderBuffer->GetBufferSize(), nullptr, &myShader);
	if (FAILED(hr))
	{
	...
	hr = DX11::myDevice->CreateInputLayout(aDesc, aDescSize, myShaderBuffer->GetBufferPointer(), myShaderBuffer->GetBufferSize(), myInputLayout.GetAddressOf());
	if (FAILED(hr))
	{
...
bool PixelShader::Init(std::wstring aFilePath)
{
	...
	// Create pixel shader
	hr = DX11::myDevice->CreatePixelShader(myShaderBuffer->GetBufferPointer(), myShaderBuffer->GetBufferSize(), nullptr, &myShader);
	if (FAILED(hr))
	{
